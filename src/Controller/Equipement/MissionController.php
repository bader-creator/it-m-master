...
class MissionController extends AbstractController
{
    ...
    public function index(Request $request, PaginatorInterface $paginator)
    {
        ...
        $mat = $this->getDoctrine()->getRepository(Materiel::class)->findMaterials();
    ...
    public function add(Request $request, PaginatorInterface $paginator)
    {
        ...
        $mat = $this->getDoctrine()->getRepository(Materiel::class)->findMaterialsByMission( $mission->getId());
        $query = $em->getRepository(Mission::class)->findBySearchInput($search_input = "");
        $materials = $this->getDoctrine()->getRepository(Materiel::class)->findAll();
        $pagination = $paginator->paginate(
            $query, /* query NOT result */
    ...
    public function show(Request $request, PaginatorInterface $paginator, $id)
    {
        ...
        $mat = $this->getDoctrine()->getRepository(Materiel::class)->findMaterialsByMission($id);
        $query = $em->getRepository(Mission::class)->findBySearchInput($search_input);
    ...
    public function addMateriel(Request $request, PaginatorInterface $paginator, ValidatorInterface $validator)
    {
        ...
        if ($request->isXmlHttpRequest()) { // pour vérifier la présence d'une requete Ajax
            ...
            foreach ($nomProduits as $key => $nomProduit) {
                ...
                // dump($material->getQuantiteSortie() );
                // dump($material->getStock()->getQuantiteRestant());
                // dump($material->getStock()->getQuantiteGenerale());
                // die();


                /*
            if( $material->getQuantiteSortie() <= $material->getStock()->getQuantiteRestant() && $material->getQuantiteSortie()  != 0 && $material->getStock()->getQuantiteRestant() != $material->getStock()->getQuantiteGenerale()){
           
                $material->getStock()->setQuantiteSortie(  $material->getQuantiteSortie()) ;
            ...
            else if( $material->getQuantiteSortie() > $material->getStock()->getQuantiteRestant()) {
               $this->addFlash('warning', 'Vous devez saisir une quantité inférieur à celle existante !'); 
           }
           */

                if ($material->getQuantiteSortie() <= $material->getStock()->getQuantiteRestant()) {
                    $material->getStock()->setQuantiteSortie($material->getQuantiteSortie() + $material->getStock()->getQuantiteSortie());
                    $material->getStock()->setQuantiteRestant($material->getStock()->getQuantiteGenerale() -  $material->getStock()->getQuantiteSortie());
                    $em->persist($material);
                ...
                // return $this->redirectToRoute('mission_page');
                // return new JsonResponse(['redirect' => $this->generateUrl('mission_page'), 'm' => $material->getId()] + @compact('id', 'nomProduits', 'quantiteSorties'));
            }
        ...
        $materials = $this->getDoctrine()->getRepository(Materiel::class)->findMaterials();

      //  $mat =  $em->getRepository(Materiel::class)->findMaterialsByMission($id);
        /*
hello
    */
    ...
    public function updateMateriel(Request $request, PaginatorInterface $paginator, $id, $id_mission)
    {
        ...
        // dump($materiel->getQuantiteSortie());
        // die;
        $matStockFirst = $materiel->getQuantiteSortie();
        $mission = $em->getRepository(Mission::class)->find($id_mission);
        if (!$materiel) {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            if ($materiel->getQuantiteSortie() <= $materiel->getStock()->getQuantiteRestant()) {
                    $matStock1 = -  $matStockFirst + $materiel->getStock()->getQuantiteSortie();
                    $materiel->getStock()->setQuantiteSortie( $matStock1 +$materiel->getQuantiteSortie());
